...
这个方法可以实现通过添加删除文件就可以为原始类型添加删除接口，还可以替换各种不同的同接口实现；并且使用者只需要引用相关头文件，不需要知道具体类型。  
缺陷是必须使用智能指针。（可能以后会优化）  
可能还有一些其它缺陷，这里只是放出这种解决方案供探讨。  
...
#define CONVERTOR(name) 	\
	template<typename TInterface, typename TImpl = InterfaceImpl<name, TInterface>>\
		inline operator std::shared_ptr<TInterface>()\
	{\
		return std::shared_ptr<TInterface>(new TImpl(shared_from_this())); \
	}\
		template<typename TInterface, typename TImpl = InterfaceImpl<name, TInterface>>\
		inline operator std::unique_ptr<TInterface>()\
	{\
		return std::unique_ptr<TInterface>(new TImpl(shared_from_this())); \
	}
```
